...
Когда команды передаются напрямую через `tty`, говорят, что интерпретатор находится в интерактивном режиме. В этом режиме он запрашивает ввод следующей команды в основной промпт, обычно указывая на место ввода тремя знаками больше: `>>>`. Для дополняющих выражений (continuation lines), интерпретатор предлагает вторичный промпт (secondary prompt), по-умолчанию используя три точки: `...`.  Запустив интерпретатор в интерактивном режиме, мы увидим в терминале сообщение с его версией, а также небольшую информационную сноску по поводу некоторых доступных команд, а также первичный промпт.   
...
Упомянутые дополняющие выражения (continuation lines) используются для ввода вложенных, многострочных конструкций. Например, для описания вложенных конструкций как циклы или условия.
