...
По умолчанию исходные файлы Python используют кодировку `UTF-8`. Данная кодировка поддерживает символы большинства естественных языков мира и как следствие данные языки могут использоваться в строках, идентификаторах и комментариях, однако стандартная библиотека использует в качестве идентификаторов исключительно символы ASCII, это обусловлено соглашением, что должен следовать любой переносимый код.

Следует заметить, чтобы в вашем текстовом редакторе все символы, поддерживаемые `UTF-8`, отображались корректно, сам редактор должен поддерживать эту кодировку, а также использовать соответствующий шрифт, также поддерживающий `UTF-8`.

Чтобы переопределить кодировку по-умолчанию, в первой строке файла можно добавить специальный комментарий:

```Python
# -*- coding: encoding -*-
```

Где `encoding` — это одна из валидных кодировок, поддерживаемых Python. Список таких кодировок можно найти в модуле [codecs](https://docs.python.org/3/library/codecs.html#module-codecs).

Например, объявить, что в файле будет использоваться кодировка Windows-1252 можно добавив следующий комментарий:

```Python
# -*- coding: cp1252 -*-
```

Также существует одно исключение, когда первой строкой в файле указывается [строка UNIX «shebang»](https://docs.python.org/3/tutorial/appendix.html#tut-scripts), в этом случае кодировка указывается второй строкой.
